...
class Connector:
                    ...
                    try:
                        ...
                        for deprecated_class in deprecated_classes:
                            if deprecated_class in content:
                                ...
                                for line in lines:
                                    line = line.strip()
                                    if (line.startswith('from ') or line.startswith('import ')) and deprecated_class in line and "DeclarativeStreamModel" not in line and "YamlDeclarativeSource" not in line:
                                        return True
    ...
    def is_eligible_for_migration(self) -> bool:
        return not self.has_custom_incremental_sync() and not self.has_custom_retriever() and not self.has_custom_partition_router() and not self.imports_deprecated_class()
    ...
    def migrate_to_cdk_v7(self, sha256_hash: str = "<TBD>") -> bool:
        ...
        if success:
            ...
            if not changelog_success:
                print(f"Warning: changelog update failed for {self.name}, but migration was successful")
        ...
        try:
            ...
            if updated:
                ...
                if not poetry_lock_success:
                    print(f"Warning: poetry lock failed for {self.name}, but pyproject.toml was updated successfully")
    ...
    def _update_changelog(self) -> bool:
        ...
        try:
            ...
            changelog_file = self.path.parent.parent.parent / "docs" / "integrations" / "sources" / f"{doc_name}.md"
...
Examples:
  ...
  python v7_migration.py --connectors-path ../connectors --count 3
    ...
    connectors = [Connector(connector_path) for connector_path in get_connector_directories(airbyte_repo_path / "airbyte-integrations" / "connectors")]
    ...
    non_eligible_for_migration = [c for c in v6_declarative_sources if not c.is_eligible_for_migration()]

    print(f"{len(eligible_for_migration)} sources eligible for automatic migration out of {len(v6_declarative_sources)} sources using CDK version 6")
