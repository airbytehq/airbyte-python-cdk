"""Metadata checks for Airbyte connectors."""
...
class MetadataCheck(Check):
    """Base class for metadata checks."""
...
class ValidateMetadata(MetadataCheck):
    ...
    description = f"Connectors must have a `{consts.METADATA_FILE_NAME}` file at the root of their directory. This file is used to build our connector registry. Its structure must follow our metadata schema."
...
class CheckConnectorLanguageTag(MetadataCheck):
    ...
    description = f"Connectors must have a language tag in their metadata. It must be set in the `tags` field in {consts.METADATA_FILE_NAME}. The values can be `language:python` or `language:java`."
...
class CheckConnectorMaxSecondsBetweenMessagesValue(MetadataCheck):
    ...
    description = "Certified source connectors must have a value filled out for `maxSecondsBetweenMessages` in metadata. This value represents the maximum number of seconds we could expect between messages for API connectors."
