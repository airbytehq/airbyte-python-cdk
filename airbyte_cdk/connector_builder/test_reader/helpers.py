...
def _close_page(
    ...
    """
    Closes the current page by encapsulating the request, response, and current page records into a StreamReadPages object,
    which is then appended to the list of slice pages. After the encapsulation, the list of current page records is cleared.
    Args:
        current_page_request (Optional[HttpRequest]): The HTTP request associated with the current page. Can be None.
        current_page_response (Optional[HttpResponse]): The HTTP response associated with the current page. Can be None.
        current_slice_pages (List[StreamReadPages]): The list that collects pages read from the stream.
        current_page_records (List[Mapping[str, Any]]): The list of records extracted from the current page which will be deep-copied
                                                      and stored in a new StreamReadPages instance before being cleared.
    Returns:
        None
    """
...
def _is_page_http_request(json_message: Optional[Dict[str, Any]]) -> bool:
    ...
    if not json_message:
        ...
        return _is_http_log(json_message) and not _is_auxiliary_http_request(json_message)
...
def _need_to_process_slice_descriptor(message: AirbyteMessage) -> bool:
    """
...
def _need_to_close_slice_page(at_least_one_page_in_group: bool, message: AirbyteMessage) -> bool:
    ...
    """
    return at_least_one_page_in_group and _need_to_process_slice_descriptor(message)


def _is_log_message(message: AirbyteMessage) -> bool:
    """
...
def _is_trace_with_error(message: AirbyteMessage) -> bool:
    """
...
def _is_record_message(message: AirbyteMessage) -> bool:
    """
...
def _is_config_update_message(message: AirbyteMessage) -> bool:
    """
...
def _is_state_message(message: AirbyteMessage) -> bool:
    """
...
def _airbyte_message_to_json(message: AirbyteMessage) -> Optional[Dict[str, JsonType]]:
    """
...
def _handle_auxiliary_request(json_message: Dict[str, JsonType]) -> AuxiliaryRequest:
    """
    ...
    return AuxiliaryRequest(
        ...
        request=_create_request_from_log_message(json_message),
        response=_create_response_from_log_message(json_message),
    )
...
def _close_current_page(
    ...
    """
    Closes the current page by invoking the internal _close_page function to properly finalize
    the page processing, and then resets both the current page request and current page response
    to None.

    Parameters:
        current_page_request (Optional[HttpRequest]): The HTTP request associated with the current page.
        current_page_response (Optional[HttpResponse]): The HTTP response obtained from processing the current page.
        current_slice_pages (List[StreamReadPages]): A list of pages corresponding to the current slice.
        current_page_records (List[Mapping[str, Any]]): A list of records extracted from the current page.

    Returns:
        tuple[None, None]: A tuple where both elements represent the current page request and response,
        now set to None after processing.
    """
