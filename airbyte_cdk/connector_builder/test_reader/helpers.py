...
class Parsers:
    ...
    def airbyte_message_to_json(message: AirbyteMessage) -> Optional[Dict[str, JsonType]]:
        ...
        """
        if Conditions.is_log_message(message):
            json_object = Parsers.parse_json(message.log)  # type: ignore
...
class Conditions:
    ...
    def should_close_page(
        ...
        return (
            ...
            and Conditions.is_log_message(message)
            and (
                Conditions.is_page_http_request(json_message)
    ...
    def should_process_slice_descriptor(message: AirbyteMessage) -> bool:
        ...
        """
        return Conditions.is_log_message(message) and message.log.message.startswith(  # type: ignore
            SliceLogger.SLICE_LOG_PREFIX
        )

    @staticmethod
    def should_close_page_for_slice(
        at_least_one_page_in_group: bool, message: AirbyteMessage
    ) -> bool:
        """
        ...
        """
        return at_least_one_page_in_group and Conditions.should_process_slice_descriptor(message)
    ...
    def is_page_http_request(json_message: Optional[Dict[str, Any]]) -> bool:
        ...
        if not json_message:
            ...
            return Conditions.is_http_log(
                json_message
            ) and not Conditions.is_auxiliary_http_request(json_message)
    ...
    def is_auxiliary_http_request(message: Optional[Dict[str, Any]]) -> bool:
        ...
        return Conditions.is_http_log(message) and message.get("http", {}).get(
            "is_auxiliary", False
        )
...
class Handlers:
    ...
    def handle_auxiliary_request(json_message: Dict[str, JsonType]) -> AuxiliaryRequest:
        ...
        return AuxiliaryRequest(
            ...
            request=Parsers.create_request_from_log_message(json_message),
            response=Parsers.create_response_from_log_message(json_message),
        )
    ...
    def handle_log_message(
        ...
        if json_message is not None and Conditions.is_http_log(json_message):
            if Conditions.is_auxiliary_http_request(json_message):
                auxiliary_request = Handlers.handle_auxiliary_request(json_message)
            else:
                at_least_one_page_in_group = True
                current_page_request = Parsers.create_request_from_log_message(json_message)
                current_page_response = Parsers.create_response_from_log_message(json_message)
        else:
