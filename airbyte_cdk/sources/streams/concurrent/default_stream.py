...
class DefaultStream(AbstractStream):
    def __init__(
        ...
        name: str,
        json_schema: Union[Mapping[str, Any], Callable[[], Mapping[str, Any]]],  # FIXME well, this is interesting: the schema wasn't loaded lazily if we passed `Mapping` here which means that if streams were doing HTTP requests to get the schema, the would do it even if the stream wasn't used during the command
