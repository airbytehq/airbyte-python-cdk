...
class _CsvReader:
    def read_data(
        ...
        try:
            with stream_reader.open_file(
                ...
                try:
                    headers, trailing_empty_header_count = (
                        self._get_headers_and_trailing_empty_count(fp, config_format, dialect_name)
                    )
                except UnicodeError:
    ...
    def _get_headers(self, fp: IOBase, config_format: CsvFormat, dialect_name: str) -> List[str]:
        headers, _ = self._get_headers_and_trailing_empty_count(fp, config_format, dialect_name)
        return headers

    def _get_headers_and_trailing_empty_count(
        ...
        if isinstance(config_format.header_definition, CsvHeaderAutogenerated):
            ...
            headers_without_trailing_empty_values = (
                headers[: len(headers) - trailing_empty_header_count]
                if trailing_empty_header_count
                else headers
            )
            interior_empty_count = sum(
                1 for h in headers_without_trailing_empty_values if self._is_empty_csv_value(h)
            )
            ...
            headers = headers_without_trailing_empty_values

        fp.seek(0)
        return headers, trailing_empty_header_count
