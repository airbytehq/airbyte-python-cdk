...
class _CsvReader:
    def read_data(
        ...
        try:
            with stream_reader.open_file(
                ...
                # Compute trailing empty header count for data row stripping.
                # _get_headers already stripped trailing empties from the returned headers,
                # but we need the count to strip corresponding values from data rows.
                trailing_empty_header_count = 0
                if not isinstance(
                    config_format.header_definition, (CsvHeaderUserProvided, CsvHeaderAutogenerated)
                ):
                    self._skip_rows(fp, config_format.skip_rows_before_header)
                    raw_reader = csv.reader(fp, dialect=dialect_name)  # type: ignore
                    raw_headers = list(next(raw_reader))
    ...
    def _get_headers(self, fp: IOBase, config_format: CsvFormat, dialect_name: str) -> List[str]:
        ...
        if isinstance(config_format.header_definition, CsvHeaderAutogenerated):
            ...
            trailing_empty_header_count = self._get_trailing_empty_header_count(headers)
            headers_without_trailing = (
                headers[: len(headers) - trailing_empty_header_count]
                if trailing_empty_header_count
                else headers
            )
            interior_empty_count = sum(
                1 for h in headers_without_trailing if self._is_empty_csv_value(h)
            )
            if interior_empty_count:
                raise AirbyteTracedException(
                    message="CSV header row contains interior empty column name(s).",
                    internal_message=f"Found {interior_empty_count} empty/whitespace-only column name(s) before trailing blank columns in header: {headers}",
                    failure_type=FailureType.config_error,
                )
            if trailing_empty_header_count == len(headers):
                raise AirbyteTracedException(
                    message="CSV header row contains only empty column name(s).",
                    internal_message=f"Found {trailing_empty_header_count} empty/whitespace-only column name(s) in header: {headers}",
                    failure_type=FailureType.config_error,
                )
            headers = headers_without_trailing
