...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("test_exponential_jitter_from_config", 2, 5, "{{config['jitter']}}", 20),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(2, 5, 5, 15, 25, id="centered_jitter"),
        pytest.param(1, 2, 10, 0, 14, id="lower_bound_clamped_to_zero"),
    ],
)
...
def test_exponential_backoff_with_negative_jitter_raises_error():
    ...
    with pytest.raises(ValueError, match="jitter_range_in_seconds"):
        backoff_strategy.backoff_time(response_mock, attempt_count=2)
