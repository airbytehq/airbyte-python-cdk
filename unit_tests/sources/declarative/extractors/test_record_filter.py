...
def _stale_record_tracking_filter(should_be_synced_per_record: List[bool]):
    cursor = Mock()
    cursor.should_be_synced.side_effect = should_be_synced_per_record
...
def test_stale_record_seen_when_record_is_filtered_out():
    record_filter_decorator = _stale_record_tracking_filter([True, False])

    assert not record_filter_decorator.stale_record_seen_on_current_page
    filtered_records = list(
        record_filter_decorator.filter_records(
            records=[{"id": 1}, {"id": 2}],
            stream_state={},
    ...
    assert record_filter_decorator.stale_record_seen_on_current_page


def test_stale_record_seen_is_reset_on_each_filter_records_call():
    record_filter_decorator = _stale_record_tracking_filter([False, True])

    list(record_filter_decorator.filter_records(records=[{"id": 1}], stream_state={}))
    assert record_filter_decorator.stale_record_seen_on_current_page

    list(record_filter_decorator.filter_records(records=[{"id": 2}], stream_state={}))
    assert not record_filter_decorator.stale_record_seen_on_current_page


def test_stale_record_seen_is_tracked_per_thread():
    import threading

    record_filter_decorator = _stale_record_tracking_filter([False])
    list(record_filter_decorator.filter_records(records=[{"id": 1}], stream_state={}))
    assert record_filter_decorator.stale_record_seen_on_current_page

    stale_record_seen_on_other_thread = []
    other_thread = threading.Thread(
        target=lambda: stale_record_seen_on_other_thread.append(
            record_filter_decorator.stale_record_seen_on_current_page
        )
    )
    other_thread.start()
    other_thread.join()

    assert stale_record_seen_on_other_thread == [False]
